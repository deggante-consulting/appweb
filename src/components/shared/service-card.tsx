...
export function ServiceCard({ service }: Readonly<{ service: Service }>) {
  return (
    <article className="dg-card flex h-full min-h-72 flex-col gap-5 p-7 transition hover:-translate-y-1 hover:shadow-[0_16px_40px_rgba(13,13,13,0.1)] sm:p-8">
      ...
      <Link
        className="mt-auto inline-flex min-h-11 items-center text-base font-extrabold text-[var(--accent-dark)]"
        href={`/services#${service.id}`}
      >
        En savoir plus <span aria-hidden="true" className="ml-2">→</span>
      </Link>
    </article>
  );
}
