    ...
    <footer className="relative overflow-hidden bg-[var(--dark)] text-white">
      ...
      <div className="dg-container relative grid gap-8 py-10 sm:py-14 md:grid-cols-[1.45fr_1fr_1fr_1.25fr] md:gap-10">
        <div className="order-2 flex flex-col gap-4 md:order-none">
          ...
          <div className="flex gap-3 text-sm leading-7 text-[var(--on-dark)]">
            <MapPin aria-hidden="true" className="mt-1.5 shrink-0" size={16} />
            <p>
        ...
        <FooterColumn className="order-4 border-white/10 md:order-none md:border-l md:pl-10" title="Zones d'intervention">
          {zones.map((zone) => (
            <span className="text-sm leading-6 text-[var(--on-dark)]" key={zone}>
              {zone}
            </span>
          ))}
        </FooterColumn>
        <FooterColumn className="order-1 rounded-[var(--radius-small)] border border-white/5 bg-[rgba(54,178,74,0.05)] p-5 md:order-none md:-my-5 md:border-l md:border-r-0 md:border-y-0 md:border-white/10 md:pl-10" title="Contact">
          ...
          <span className="flex items-center gap-3 text-sm text-[var(--on-dark)]">
      ...
      <div className="relative border-t border-white/10">
        <div className="dg-container flex flex-col gap-4 py-5 text-[13px] text-[var(--on-dark)] sm:flex-row sm:items-center sm:justify-between sm:text-sm">
...
function FooterColumn({
  ...
  return (
    <div className={`flex flex-col gap-3 ${className}`}>
      <h2 className="text-xs font-extrabold uppercase tracking-[0.12em] text-[var(--on-dark)]">
        {title}
      </h2>
      {children}
    </div>
